...
local function sendCrate(crateInfo, sendType)
    ...
    ChatThrottleLib:SendAddonMessage("NORMAL",  "WarSupplyTimer", message, "GUILD") --"CHANNEL", "WarSupplyTimer");
end
...
local function announceCrate(crateInfo)
    if shouldAnnounce(crateInfo) and checkDelta(crateInfo) then
        ...
        PlaySoundFile("Interface\\AddOns\\WarSupplyTimer\\shipswhistle.ogg", "Master")
        print(NS.MSG_CRATE_SPOT:format(crateInfo.zoneName, crateInfo.zoneParentName, crateInfo.spotter, crateInfo.method))
        local gomsg = "War Crate has been spotted!"
            SendChatMessage(gomsg, IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and 'INSTANCE_CHAT' or IsInRaid() and 'RAID' or IsInGroup() and 'PARTY')
            SendChatMessage("has spotted a War Crate!", "EMOTE")
    end
end
