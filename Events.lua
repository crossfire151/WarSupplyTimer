...
local function updateFrame(curTime)
    ...
    if labelText == "" and timerText == "" then
        labelText = "To begin tracking War Crates\nPlease wait for the first one to drop (about 20m wait)."
    end
...
local function OnEvent(self, event, ...)
    ...
    elseif event == "ADDON_LOADED" then
        local addon = ...
        if addon == "WarSupplyTimer" then
            print("WarSupplyTimer loaded! /ws to toggle window")
            C_ChatInfo.RegisterAddonMessagePrefix("WarSupplyTimer")
            if crateDB == nil then
